...
If `.codex/hooks.json` has the AI-DDP hooks installed and trusted (see
`harness/codex/README.md`), attempts to write into a later phase's output
directory before its gate is approved will be **mechanically blocked**, not
just discouraged by these instructions. If a write gets blocked, that's the
system working correctly — tell the user their gate needs approval, don't
try to route around it (e.g., by writing to a different path to dodge the
check).
